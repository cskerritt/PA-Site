...
def head(page):
    ...
    # Per-page geo signals (default to the St. Louis office; location pages override).
    geo_region = page.get("geo_region", "US-" + SITE["region"])
    geo_place = page.get("geo_placename", f"{SITE['city']}, {SITE['region_full']}")
    geo_lines = [
        f'<meta name="geo.region" content="{geo_region}">',
        f'<meta name="geo.placename" content="{esc(geo_place)}">',
    ]
    if page.get("geo_coords", True):  # only the office-anchored pages carry coordinates
        geo_lines.append(f'<meta name="geo.position" content="{SITE["geo_lat"]};{SITE["geo_lng"]}">')
        geo_lines.append(f'<meta name="ICBM" content="{SITE["geo_lat"]}, {SITE["geo_lng"]}">')
    geo_block = "\n".join(geo_lines)
...
def build_pages():
    ...
    for st in STATES:
        ...
        pages.append(dict(path=st["path"],
                          ...
                          active="/locations/", body=b, schema=s,
                          geo_region="US-" + st["abbr"], geo_placename=st["name"], geo_coords=False,
        ...
        for idx, city in enumerate(st["cities"]):
            ...
            pages.append(dict(path=city["path"],
                              ...
                              geo_placename=f"{city['name']}, {st['name']}", geo_coords=False,
